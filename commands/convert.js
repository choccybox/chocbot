...
const quickdesc =
  "converts a file to different format based on uploaded file type or specified in the command.";
...
module.exports = {
  ...
  help: {
    description:
      "Converts a file to a different format based on uploaded file type or specified format",
    ...
    options:
      "**format:** Target format - Supports image, video, and audio formats",
  },
  buildSlashMessage: function buildSlashMessage(interaction) {
    const file = interaction.options.getAttachment("file");
    const format = interaction.options.getString("format");
    return {
      content: format ? `convert:${format}` : "convert",
  ...
  run: async function handleMessage(
    ...
    const firstAttachment =
      hasAttachment.size > 0
        ? hasAttachment instanceof Map
          ? Array.from(hasAttachment.values())[0]
          : hasAttachment.first()
        : null;
    if (!firstAttachment) {
      return message.reply({
    ...
    if (commandParts.length > 1) {
      ...
      const validFormats = [
        ...new Set([...imageTypes, ...audioTypes, ...videoTypes]),
      ];

      if (!validFormats.includes(conversionFormat)) {
        return message.reply({
          content: `Invalid or unsupported format: ${conversionFormat}`,
        });
      }

      if (conversionFormat === currentFormat) {
        return message.reply({
          content: `File is already in ${conversionFormat} format`,
        });
      }
      ...
      if (
        fileType === "video" &&
        imageTypes.includes(conversionFormat) &&
        conversionFormat !== "gif"
      ) {
        return message.reply({
          content: `Converting video to image isn't supported`,
        });
      }

      const outputFilePath = `temp/${randomName}-CONVDONE-${rnd5dig}.${conversionFormat}`;
      console.log(
        `User ${message.author.tag} chose to convert to ${conversionFormat}`,
      );
      ...
      try {
        message
          ...
          .catch(() => message.react("👍"));
        console.log(
          `converting from **${currentFormat}** to **${conversionFormat}** (file name: ${filePath} -> ${outputFilePath})`,
        );
        const response = await conversionDecider.conversionDecider(
          message,
          filePath,
          outputFilePath,
          conversionFormat,
        ...
        if (!response || !response.success) {
          return message.reply({
            content:
              response?.message || "Conversion failed. Please try again.",
          });
        }
        ...
        if (isFileTooLarge) {
          ...
          await message.reply({
            ...
            files: [
              { attachment: fileData, name: outputFilePath.split("/").pop() },
            ],
          });
        }
        ...
        setTimeout(() => {
          ...
          const patternConvDone = new RegExp(
            `${randomName}-CONVDONE-${rnd5dig}`,
          );
    ...
    if (buttons.length === 0) {
      return message.reply({
        content: "No conversion options available for this file type.",
      });
    }
    ...
    const collector = message.channel.createMessageComponentCollector({
      filter: (i) =>
        i.customId.startsWith("convert_") &&
