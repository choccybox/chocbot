...
const quickdesc =
  "download video/audio from social/music platforms (YouTube, Twitter, Instagram, SoundCLoud, Spotify) *Spotify may not result with a correct song if its not found on youtube.*";
...
module.exports = {
  ...
  help: {
    description:
      "Download video/audio from social/music platforms (YouTube, Twitter, Instagram, TikTok, SoundCloud, Spotify)",
    usage: "/download <url> [format]",
    examples: [
      "/download https://www.youtube.com/watch?v=dQw4w9WgXcQ",
      "/download https://www.youtube.com/watch?v=dQw4w9WgXcQ format:Audio",
      "/download https://www.youtube.com/playlist?list=...",
    ],
    options:
      "**format:** Video or Audio - Choose to download video or extract audio only",
  },
  buildSlashMessage: function buildSlashMessage(interaction) {
    const url = interaction.options.getString("url");
    const format = interaction.options.getString("format");
    return {
      content: format === "audio" ? `download:aud ${url}` : `download ${url}`,
    };
  },
  run: async function handleMessage(message, client, isChained) {
    ...
    const messageContentWithoutCommand = commandParts
      .slice(1)
    ...
    if (!hasContent && !hasLinks) {
      ...
      try {
        ...
        const isPlaylist =
          downloadLink.includes("list=") || downloadLink.includes("/playlist");
          ...
          if (response.message) {
            ...
            if (message.editReply) {
              return message.editReply({
                content: `❌ I wasn't able to download this video. The source might be unavailable or restricted.`,
              });
            }
            return message.reply({
              content: `I wasn't able to download this video. The source might be unavailable or restricted.`,
        ...
        if (response.success) {
          ...
          if (response.isPlaylist) {
            ...
            if (!fs.existsSync(filePath)) {
              if (message.editReply) {
                return message.editReply({
                  content: "❌ Playlist file not found.",
                });
              }
            ...
            const deleteDelay =
              (parseInt(process.env.FILE_DELETE_TIMEOUT) || 30) * 60000; // Convert minutes to milliseconds
            ...
            try {
              ...
              if (!hasAudio) {
                ...
                const promptMsg = await message.reply({
                  content:
                    "This video has no audio track. Would you like to convert it to a GIF?",
                  components: [row],
                });
                ...
                try {
                  ...
                  if (interaction.customId === "convert_to_gif") {
                    const convertMsg = await interaction.update({
                    ...
                    const palettePath = filePath.replace(
                      ".mp4",
                      "_palette.png",
                    );

                    // Get original video dimensions
                    const { stdout: dimensions } = await execPromise(
                    ...
                    while (attempts < maxAttempts) {
                      ...
                      await execPromise(
                        `ffmpeg -y -i "${filePath}" -vf "fps=15,scale=-1:${scale}:flags=lanczos,palettegen=stats_mode=diff" "${palettePath}"`,
                      );

                      // Generate GIF using palette
                      await execPromise(
                        `ffmpeg -y -i "${filePath}" -i "${palettePath}" -filter_complex "fps=15,scale=-1:${scale}:flags=lanczos[x];[x][1:v]paletteuse=dither=bayer:bayer_scale=5" "${gifPath}"`,
                      );
                    ...
                    await interaction.update({
                      content: "Keeping as video.",
                      components: [],
                    });
                  }
                } catch (err) {
                  await promptMsg.edit({
                    content: "No response. Sending as video.",
                    components: [],
                  });
                }
              }
          ...
          if (message.editReply) {
            await message.editReply({
              content: "❌ Error sending URL to downloader.",
            });
          } else {
        ...
        if (message.editReply) {
          await message.editReply({
            content: "❌ Error processing download request.",
          });
        } else {
          message.reply({ content: "Error sending URL to downloader.js." });
        }
