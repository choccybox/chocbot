...
const quickdesc = 'Creates an Anthony Bourdain quote image with two custom text fields';
...
const BASE_IMAGE_PATH = path.join(__dirname, '..', '..', 'images', 'anthony bourdain quote.png');
...
function getSlashText(message) {
  if (message.abquoteText1 !== undefined || message.abquoteText2 !== undefined) {
    return {
  ...
  const match = message.content.match(/^abquote\s+([\s\S]*?)\s+\|\s+([\s\S]*)$/i);
  if (!match) {
...
async function createABQuoteImage(text1, text2, outputPath) {
  ...
  for (const field of TEXT_FIELDS) {
    ...
    const scaleX = measuredWidth > field.maxWidth ? field.maxWidth / measuredWidth : 1;
...
module.exports = {
  run: async function handleMessage(message) {
    try {
      ...
      const outputPath = path.join('temp', `${userName}-ABQUOTE-${rnd5dig}.png`);

      await message.react('<a:pukekospin:1311021344149868555>').catch(() => message.react('👍'));
