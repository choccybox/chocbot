...
const quickdesc =
  "Overlays your or specified user's pfp and username/nickname onto the GTA iFruit call screen";
...
module.exports = {
  ...
  buildSlashMessage: function buildSlashMessage(interaction) {
    ...
    const users = user ? new Map([[user.id, user]]) : new Map();
    return {
      content: ["ifruit", user ? `<@${user.id}>` : "", customname || ""]
        .filter(Boolean)
        .join(" "),
      mentions: {
        users,
        has: (mentionedUser) => users.has(mentionedUser.id),
      },
    };
  },
  run: async function handleMessage(message) {
    ...
    try {
      ...
      const contentWithoutCommand = message.content
        .replace(commandPattern, "")
        .trim();
      const contentWithoutMentions = contentWithoutCommand
        .replace(/<@!?\d+>/g, "")
        .trim();
      ...
      const avatarURL = pfpUser.displayAvatarURL({
        extension: "png",
        size: 256,
      });
      ...
      // Convert to GIF
      await sharp(compositeImage, { animated: true }).gif().toFile(outputPath);
